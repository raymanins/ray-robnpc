    ...
    while true do
        ...
        if IsPlayerFreeAiming(PlayerId()) then
            ...
            if DoesEntityExist(targetEntity) and IsEntityAPed(targetEntity) and not IsPedAPlayer(targetEntity) and not IsPedInAnyVehicle(targetEntity) then
                ...
                -- Check if the ped is blacklisted, dead or too far
                if distance <= Config.MaxDistance then
                    ...
                    elseif IsPedDeadOrDying(targetPed, true) then
                        if aimedAtPed ~= targetPed then
                            local currentTime = GetGameTimer()
                            if currentTime - lastNotificationTime > Config.NotificationCooldown then
                                QBCore.Functions.Notify("You cannot rob a dead NPC!", "error")
                                lastNotificationTime = currentTime
                            end
                            aimedAtPed = targetPed
                        end
                ...
                -- If the player stops aiming at the ped, unfreeze the ped and make it run away
                if aimedAtPed and not progress and not robbedPeds[aimedAtPed] then
                    FreezeEntityPosition(aimedAtPed, false)
                    TaskSmartFleePed(aimedAtPed, playerPed, 100.0, -1, true, true)
                    aimedAtPed = nil
                end
            end
        else
